...
func TestFilterOptions_Validation(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "config home option",
			option:   WithConfigHome("/test/.kitops"),
			expected: "/test/.kitops",
			field:    "configHome",
		},
		{
			name:     "filters option",
			option:   WithFilters("model", "datasets"),
			expected: []string{"model", "datasets"},
			field:    "filters",
		},
		{
			name:     "overwrite option true",
			option:   WithOverwrite(true),
			expected: true,
			field:    "overwrite",
		},
		{
			name:     "overwrite option false",
			option:   WithOverwrite(false),
			expected: false,
			field:    "overwrite",
		},
		{
			name:     "ignore existing option",
			option:   WithIgnoreExisting(true),
			expected: true,
			field:    "ignoreExisting",
		},
		{
			name:     "network options",
			option:   WithNetworkOptions(&options.NetworkOptions{}),
			expected: &options.NetworkOptions{},
			field:    "networkOptions",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			switch tt.field {
			case "configHome":
				assert.Equal(t, tt.expected, config.configHome)
			case "filters":
				assert.Equal(t, tt.expected, config.filters)
			case "overwrite":
				assert.Equal(t, tt.expected, config.overwrite)
			case "ignoreExisting":
				assert.Equal(t, tt.expected, config.ignoreExisting)
			case "networkOptions":
				assert.Equal(t, tt.expected, config.networkOptions)
			default:
				t.Errorf("Unknown field: %s", tt.field)
			}
		})
	}
}
